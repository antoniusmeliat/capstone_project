...
param(
    ...
    [ValidateRange(1, 99)]
    [int]$previous_wkNum= 12,

    [Parameter(Mandatory = $false)]
    [ValidateRange(1, 99)]
    [int]$current_wkNum= 13
)
...
$destCapstoneRoot = Join-Path $scriptDir ("runs\week_$current_wkNum")
$prev_FilePath = Join-Path $scriptDir ("runs\week_$previous_wkNum")
#$destWeekFolder   = Join-Path $destCapstoneRoot
#Write-Host $destCapstoneRoot
...
if (-not (Test-Path -Path $destCapstoneRoot)) {
    ...
    Write-Error ("Destination path not found: {0}" -f $destCapstoneRoot)
    exit 1
}
...
Write-Host ("Source week folder     : {0}" -f $prev_FilePath)   -ForegroundColor Cyan

# Validate source base
if (-not (Test-Path -Path $prev_FilePath)) {
    Write-Error ("Source  path not found: {0}" -f $prev_FilePath)
    exit 1
}
...
for ($folder_counter = 1; $folder_counter -le 8; $folder_counter++) {
    ...
    $srcNewInputs  = Join-Path $srcFolder ("inputs\new_inputs.npy")
    $srcNewOutputs = Join-Path $srcFolder ("inputs\new_outputs.npy")
    $srcParams = Join-Path $srcFolder ("inputs\params.yaml")

    $destFunctionFolder = Join-Path $destCapstoneRoot $current_folder
    $destInitInputs     = Join-Path $destFunctionFolder ("inputs\initial_inputs.npy")
    $destInitOutputs    = Join-Path $destFunctionFolder ("inputs\initial_outputs.npy")
    $destParams = Join-Path $destFunctionFolder ("inputs\params.yaml")
    ...
    # Copy params.yaml
    if (Test-Path -Path $srcParams) {
        try {
            Copy-Item -Path $srcParams -Destination  $destParams -Force
            Write-Host ("Copied {0}: params.yaml" -f $current_folder) -ForegroundColor Green
        }
        catch {
            Write-Warning ("Failed to copy params for {0}. Error: {1}" -f $current_folder, $_.Exception.Message)
        }
    }
    else {
        Write-Warning ("Source params not found for {0}: {1}" -f $current_folder, $srcNewOutputs)
    }
